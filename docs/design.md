...
- **Don't** attach verbose explanations, parenthetical notes, or descriptive commentary directly inside button labels or menu items (e.g., use `"Đặt lại"` instead of `"Đặt lại (Chưa làm)"`, `"Chỉ lưu ghi chú"` instead of `"Chỉ lưu ghi chú (Không đổi lịch)"`). Action labels must remain strictly concise, punchy, and functional. If an action requires explanation, place it in dedicated helper text or an explicit contextual note section—never inside the action trigger.
- **Don't** duplicate external navigation links (e.g., LeetCode/NeetCode tags) inside the action dropdown menu if they already exist on the card or table row. The action dropdown must remain focused exclusively on problem-state mutations and primary modal actions.
